...
fig_platform = px.bar(platform_usage, x='Platform', y='Hours', title="Hours Played by Platform")
...
time_series = df.groupby('date')['hours'].sum().reset_index()
time_series['hours'] = time_series['hours'].round(2)
fig_time = px.line(time_series, x='date', y='hours', title="Daily Listening Time (Hours)")
